// corekit/contracts/schemas/definition.mjs — Fleet Definition content (Foundation-owned schemas)
//
...
export const POLICY_SCHEMA = definitionSchema('policy', 1, {
  name: { type: 'string', required: true, minLength: 1, maxLength: 80 },
  domain: {
    type: 'string', required: true, enum: ['model', 'memory', 'cost', 'rollout', 'approval'],
    describe: 'Which platform-bounded knob set this policy tunes',
  },
  settings: {
    type: 'object', required: true, open: true,
    describe: 'Domain-specific values. The compiler clamps them to platform bounds — a policy cannot exceed what Foundation permits (C-33).',
  },
});

// ── Project template ───────────────────────────────────────────────────

export const PROJECT_TEMPLATE_SCHEMA = definitionSchema('projectTemplate', 1, {
  name: { type: 'string', required: true, minLength: 1, maxLength: 80 },
  goal_hint: { type: 'string', maxLength: 500 },
  default_team_roles: { type: 'array', items: { type: 'string', pattern: ID_PATTERN }, default: [] },
  standard_processes: { type: 'array', items: { type: 'string', pattern: ID_PATTERN }, default: [] },
  context_keys: { type: 'array', items: { type: 'string' }, default: [], describe: 'Context entries a new project of this kind should carry' },
});

// ── Eval suite ─────────────────────────────────────────────────────────

export const EVAL_SUITE_SCHEMA = definitionSchema('evalSuite', 1, {
  name: { type: 'string', required: true, minLength: 1, maxLength: 80 },
  target: {
    type: 'object', required: true,
    properties: {
      kind: { type: 'string', required: true, enum: ['role', 'skill', 'process'] },
  ...
  cases: {
    ...
    items: {
      ...
      properties: {
        id: { type: 'string', required: true, pattern: ID_PATTERN },
        instruction: { type: 'string', required: true, minLength: 5 },
        fixtures: { type: 'object', open: true, default: {} },
        expect: { type: 'string', required: true, describe: 'What a passing run produces — the observable outcome, never a tool detail' },
        grader: { type: 'string', enum: ['criteria', 'exact', 'rubric'], default: 'criteria' },
        origin: {
          type: 'string', enum: ['authored', 'regression', 'replay'], default: 'authored',
          describe: 'regression cases come from real corrections; replay from sanitized history',
        },
      },
    },
  },
  thresholds: {
    type: 'object',
    describe: 'Rollout gates. A candidate below these does not promote (C-31).',
    properties: {
      min_pass_rate: { type: 'number', min: 0, max: 1, default: 0.9 },
      max_regression: { type: 'number', min: 0, max: 1, default: 0 },
      max_cost_increase: { type: 'number', min: 0, default: 0.25 },
    },
  },
});

...
- **Human-Triggered Improvement Loop & Install Hardening (v2026.06.28.3.0)**:
  ...
  - **Evidence-based improvement modules**: 4 improvement module processes (`p-improve-skills`, `p-improve-souls`, `p-improve-work-layer`, `p-improve-context`) rewritten to v2 — derive evidence from `work-log-read` mission trees and `brain-telemetry-read` dispatch data instead of phantom aggregated counters. (Note: `p-improve-delegation` was rewritten to v2 here but later deleted in the tiered model migration — see v2026.06.28.5.0.)

...
## Current Architecture (v2026.07.04.1.0)
...
- **Drive Workspace Standard (v2026.06.25.1.0)**:
  ...
  - **Fleet-wide enforcement**: All 9 motor SOUL_APPEND files include "Workspace Convention" block covering git workspace (primary, automatic) and Drive workspace (stakeholder-facing). All 9 cerebellum SOUL_APPEND files include "Workspace Convention Gate" for verification. All 10 specialty `kit.json` files include `workspace-git` in `base_skills`.
...
- **Culture of Work primitives**: Nine primitives form the work hierarchy: **Task** (atomic execution), **Checkpoint** (task group), **Mission** (self-contained goal), **Project** (recursive organizer with context), **Process** (reusable template), **Plan** (unexecuted Mission blueprint: draft→approved→executing→complete), **Responsibility** (scheduled/event-triggered work), **Artifact** (files produced during Missions, dual-substrate: git ether primary + Drive stakeholder-facing), **Skill** (reusable execution instruction package). Every Mission requires `project_id` (defaults to `{agent-id}/general`). Missions support `depends_on` for dependency management (auto-activation on completion). Plans are created via `createPlan()`, approved via `approvePlan()`, and stamped into M→C→T envelopes via `stampPlan()`. Projects are recursive (max depth 4) with accumulated context (documentation, processes, team, configuration). Artifacts live in git repos (GCS bundles + Firestore CAS refs) per project; also published to Drive for stakeholder access; manifest written to envelope context on completion.
- **Git Artifact Substrate (C-24)**: Work products stored in git repos backed by GCS bundles (objects) + Firestore CAS refs (branches). Two planes: Plane 1 (GitHub) is the template source (architect-prime deployed via manifests), Plane 2 (GCS+Firestore) is the shared agentic ether. One repo per project. Mission lifecycle: `initWorkspace` clones+branches (`mission/{missionId}`), checkpoint completion commits+syncs, mission completion merges to `main` + builds manifest. Objects-before-refs ordering (C-18 parallel) ensures crash-safe writes. Transport: `corekit/lib/git-store.mjs`. Motor atoms: `skills/workspace-git/` (work-clone, work-branch, work-commit, work-sync, work-merge, work-status, work-diff, work-log). Config: `contracts.json` git block (`${TENANT}-agent-git` bucket, 5 push retries, 25 gc bundle threshold). Weekly GC responsibility (`r-git-gc`) prunes unreferenced bundles > 30 days.

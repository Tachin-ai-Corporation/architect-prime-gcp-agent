...
export async function handleCheckpointPlan(ctx, deps) {
  ...
  if (PROCESSES && Object.keys(PROCESSES).length > 0) {
    ...
    if (matchingProcesses.length > 0) {
      ...
      // Usage signal for the post-mission lesson reflex (context-maintenance.mjs): record which playbooks
      // were surfaced to the planner for this work, so temporal-memory can record what the mission taught
      // about them — into memory, never into the playbook. MERGED, not overwritten: a responsibility's
      // processRef playbook is stamped at fire time and must survive the planner's own keyword match.
      try {
        if (envelope) {
          envelope.recalled_processes = [...new Set([...(envelope.recalled_processes || []), ...matchingProcesses.map(p => p.id)])];
        }
      } catch { /* best-effort */ }

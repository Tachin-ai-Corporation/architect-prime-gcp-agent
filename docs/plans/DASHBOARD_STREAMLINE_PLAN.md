...
> **Version:** 0.2
> **Status:** IN IMPLEMENTATION — decisions locked (2026-07-24): **Cost tab REMOVED entirely** (not just the `$` — delete `CostDashboard` + its agent-hub tab); **IA = tabs-primary, standalone routes are drill-in detail only** (tab renders the canonical ListView; delete `/p/[id]/fleet`, collapse `/p/[id]/work`, merge the two `projects` routes); **full P1–P5** in one sustained pass. Theme stays dark-only (token sweep just makes a future light mode *possible*).

...
>
> **Implementation status — SHIPPED to repo `v2026.08.21.4.1`–`.4.5` (build-green; operator deploys + verifies live, as the dashboard is auth-gated).** P0 work-tree fix ✓ · P1 chat-first / agent-chat retired / Models→Brain / dead-code + honest copy ✓ · P2 global Approvals drawer+badge ✓ · P3 node-graph observability Home + Fleet Studio absorbed ✓ · P4 real-time chat rebuild + Settings-onto-hash-tabs + honest breadcrumb ✓. **Deferred, with reason:** C4 (invisible dead payload); R2 (three working config viewers — regression-risky merge, not requested); R3 (fleet-grid dedup — both work); deep R4 (projects/processes tab-vs-route collapse); and the **full path-based nested-route migration** — decision 2 was met via the hash-tab unification + honest breadcrumb instead, because a ~16-file blind structural rewrite (no local auth to verify) carries regression risk out of proportion to its gain. Path-routes remain available as a **verify-first follow-up**.

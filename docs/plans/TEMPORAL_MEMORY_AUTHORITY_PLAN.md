...
> **Version:** 0.1
> **Status:** IMPLEMENTED & DEPLOYED fleet-wide (2026-07-23, v2026.07.23.1.0–.6.0). All phases shipped to all 7 agents; canary Millie verified — services active, clean boot, every layer landed (config exec-gate, temporal-memory owns the memory CLIs, process → temporal-memory, SOUL = memory authority, `--importance` write + value-ranked read). Recall hot path is unchanged (still daemon-prefetched/toolless — the exec split preserved it), so fleet operation is unaffected. **Behavioral proof LANDED (2026-07-23, v2026.07.23.11.0–.12.0):** using the new on-demand responsibility trigger ([RESPONSIBILITY_TRIGGER_PLAN](RESPONSIBILITY_TRIGGER_PLAN.md)) to fire consolidation out of turn, it now **completes cleanly 9/9** — the linked process runs deterministically via `executeProcess`, temporal-memory executes every step **tooled**, per-checkpoint cerebellum PASSes, `consolidation_report.md` is written, and MEMORY.md is pruned — no block, no verify churn. This required three additional fixes the on-demand fire exposed: the scheduler pre-set `process_id` and routed to Cortex, tripping `follow_process`'s already-executed guard so the process never ran (fixed: don't pre-set → Cortex→`executeProcess`); checkpoint task dispatch set no `_exec`, so temporal-memory ran toolless in checkpoints (fixed: `_exec:true` on the checkpoint payload); the singleton guard treated `archived` as non-terminal (fixed: in-progress allowlist scoped to M). Verified + rolled fleet-wide 7/7. Grounded in last night's blocked consolidation (mission `w-1784793653477-f684c07c`, 2026-07-23 08:00 UTC): `p-memory-consolidate` pruned MEMORY.md (2,049→604 chars) but **Core Memory reconciliation and Deep Truths review never ran** — cortex's re-planning routed them to `temporal-memory`, which is **toolless**, so it stalled (*"cannot retrieve Core Memory data"*) and the mission **blocked** after ~4 min of verification churn.

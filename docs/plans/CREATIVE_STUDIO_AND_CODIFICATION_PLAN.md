...
- `30 15 * * 4` = **Thu 15:30 UTC = Thu 10:30 CDT** (dow `4` = Thursday; lands after the week's meetings). **Cron is UTC-only — the `timezone` field is documentation, not honored** (`platform/work/scheduler.mjs`); when DST ends it fires at 09:30 CST, so re-pin to `30 16 * * 4` at the boundary if the exact hour matters.

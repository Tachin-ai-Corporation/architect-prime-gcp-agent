...
**Worse looks like:** memory that only grows; stale facts shaping live decisions; promotion without evidence; an agent whose behavior drifts because its memory did; a consolidation pass editing a playbook or a project record; a load-bearing binding kept in memory, where pruning can remove it.
...
| **Temporal-Memory** | Judgment + memory-scoped effects | Curate the agent's memory: recall high-value knowledge on demand; record & reconcile it in consolidation (one job, two modes) | Reaches beyond memory — web (Research), arbitrary/Workspace mutation (Motor), definitions (processes, projects, skills: read, never written); invents facts |
...
### B-37 · The verdict is graded, and the verifier owns it
The cerebellum's judgment of a milestone is not a two-way switch. It renders one of three grades against the milestone's INTENT, judged from the full evidence and the artifact itself (B-28): **met** (clean); **met-with-caveat** (the intent is achieved, but a listed criterion is partially met or deferred in a way that does NOT defeat the deliverable — a value that resolves at runtime, an optional enrichment left undone — and the gap is surfaced as an honest, operator-facing caveat, never hidden); and **not-met** (the intent is genuinely unmet: wrong output, a missing core deliverable, an unrecoverable error, a claim contradicted by the evidence). The grade is expressed **structurally**, never parsed from prose — the caveat rides the pass verdict as a field, so a clean pass stays byte-identical and the flow machinery is untouched. This is the brain-side of C-38: a functional-but-imperfect deliverable passes WITH its caveat rather than being forced through the binary into a false block, and the deterministic guards shrink to an honesty floor (a deliverable must exist) instead of standing in as the judge. The retained guardrails keep the rebalance honest — the adversarial pass still runs over a pass at consequential stakes, and a defeating gap is still not-met.
**Better looks like:** a verifier that passes a registered, working playbook and notes "two source folders resolve at runtime" as a caveat the operator sees; a completion whose single surfaced caveat is the whole difference between honest and blocked; the block guard asking the cerebellum "does this deliverable meet the goal?" rather than tallying success rows.
**Worse looks like:** a milestone failed because the third of three criteria is imperfect though the deliverable works; a caveat used to wave through a deliverable that is actually wrong (a defeating gap dressed as a caveat); a caveat swallowed silently so the operator never learns of the gap; a third deterministic gate bolted on to catch what the last one mis-judged.

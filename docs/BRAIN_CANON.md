...
The recent conversation transcript is a sliding-window situational context, never represented as agent memory. It is assembled deterministically by the daemon, trimmed to a character budget, and injected as a structured turn transcript into the intake, classify, and decide phases. Organs never fetch or query the chat history themselves — they operate within the situational envelope context constructed for them by the harness. Sub-agent dispatches do not inherit the conversation (B-4): motor receives instructions, not chatter. Conversation content earns persistence only through the same consolidation gate as everything else (B-5) — never promoted as-is. In multi-channel environments (such as Google Chat), the transport adapter (ears) serializes this thread-scoped context directly onto the intake document at poll-time, bypassing downstream database assembly hops.

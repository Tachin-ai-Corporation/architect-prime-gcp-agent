...
/**
 * GET /api/primes/[id]/projects/[projectId]/promotions — List promotion candidates
 *
 * Query params:
...
export async function GET(req: NextRequest, ctx: RouteContext) {
  try {
    ...
    let query = promotionsCol(id, projectId)
      .orderBy("suggested_at", "desc");

    if (statusFilter) {
      query = promotionsCol(id, projectId)
...
/**
 * POST /api/primes/[id]/projects/[projectId]/promotions — Accept or dismiss
 *
...
export async function POST(req: NextRequest, ctx: RouteContext) {
  ...
  try {
    ...
    const promoRef = promotionsCol(id, projectId).doc(promotionId);
    ...
    if (action === "accept" && promoData.key && promoData.entry) {
      const projectRef = projectsCol(id).doc(projectId);

...
export function FleetPanel({ primeId }: FleetPanelProps) {
  ...
  const handleFire = useCallback(
    async (agentName: string) => {
      ...
      const res = await api<{ success: boolean }>(`/api/primes/${primeId}/fleet/fire`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className={styles.fleetPanel}>
      <div className={styles.fleetHeader}>
        <div>
          <h2 className={styles.fleetTitle}>Fleet — {prime?.name || primeId}</h2>
